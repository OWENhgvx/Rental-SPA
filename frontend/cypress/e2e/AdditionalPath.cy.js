...
describe('Guest booking status update via notifications', () => {
    it('shows acceptance notification', () => {
      ...
      // Step A — mock GET /bookings (same structure style as step 1–4)
      cy.intercept('GET', '**/bookings', {
        statusCode: 200,
        body: {
          bookings: [
      ...
      // Step B — mock GET /listings  (host poll, like step 1–4)
      cy.intercept('GET', 'http://localhost:5005/listings', {
        statusCode: 200,
        body: { listings: [] },
      }).as('loadAllListings');
  
      // Step C — visit homepage with initial localStorage (same style)
      ...
      // Step D — wait for the two API calls (matches the structure of step 1–4)
      cy.wait('@pollBookingsLoop');
      cy.wait('@loadAllListings');
  
      // Step E — verify notification text (same assertion style)
